...
def seed_db_data(db: Session):
    """
    Seeds store, category, product, and historical sales data from CSVs.
    """
    base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    
    # 1. Seed StoreMaster
    if db.query(StoreMaster).count() == 0:
        store_csv = os.path.join(base_dir, "..", "data", "raw", "KiranaIQ_All_5_Stores_Dataset", "store_master.csv")
    ...
    # 2. Seed CategoryMaster
    if db.query(CategoryMaster).count() == 0:
        cat_csv = os.path.join(base_dir, "..", "data", "raw", "KiranaIQ_All_5_Stores_Dataset", "category_master.csv")
        if os.path.exists(cat_csv):
            df = pd.read_csv(cat_csv)
            for _, row in df.iterrows():
                db.add(CategoryMaster(
                    category_id=int(row['category_id']),
                    category_name=row['category_name']
                ))
            db.commit()

    # 3. Seed ProductMaster
    if db.query(ProductMaster).count() == 0:
        prod_csv = os.path.join(base_dir, "..", "data", "raw", "KiranaIQ_All_5_Stores_Dataset", "product_master.csv")
    ...
    # 4. Seed HistoricalSale
    if db.query(HistoricalSale).count() == 0:
        sales_csv = os.path.join(base_dir, "..", "data", "processed", "kirana_features.csv")
        if os.path.exists(sales_csv):
            df = pd.read_csv(sales_csv).head(2000)
            for _, row in df.iterrows():
                db.add(HistoricalSale(
                    ...
                    stock_available=int(row['stock_available']) if pd.notna(row['stock_available']) else 0,
                    is_weekend=int(row['is_weekend']) if pd.notna(row['is_weekend']) else 0,
                    is_festival_near=int(row['is_festival_near']) if pd.notna(row['is_festival_near']) else 0
                ))
            db.commit()
